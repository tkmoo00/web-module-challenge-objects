...
function createMenuItem(val1, val2, val3) {
  let obj = {
    name: val1,
    price: val2,
    category: val3,
...
const JuliusFeedback = reviews[5].feedback;
...
//   if (reviews.name === "Reyna") {
//     renyaReviews.push(reviews[i]);
//     reviews.feedback =
...
function getLastReview(arrObj) {
  return `${arrObj[arrObj.length - 1].name} gave the restaurant a ${
    arrObj[arrObj.length - 1].rating
  } star review, and their feedback was: ${arrObj[arrObj.length - 1].feedback}`;
}
...
function getReviewByRating(revArr) {
  // revArr = [];
  // for (let i = 0; i <= reviews.length; i++) {
  //   if (reviews[i].rating === 4) {
  //     revArr.push(reviews[i]);
  //   }
  // }
  // return revArr;
}
